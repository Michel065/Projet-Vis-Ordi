...
def run_mae(path="outputs/model/MAE.pth", load=True, do_train=False):
    ...
    model = MAE(img_size=96, patch_size=8, in_channels=3, embed_dim=768, decoder_embed_dim=576) #576 sinon probleme avec les heads
    ...
    if do_train:
        ...
        loss_history = train(model, dataloader_train, optimizer, device, epochs=25, patch_size=8)
...
def run_ae(path="outputs/model/AE.pth", load=True, do_train =False):
    ...
    model = AE(img_size=96, patch_size=8, in_channels=3, embed_dim=768, decoder_embed_dim=576)
...
def run_ae_load_eval_classification(path="outputs/model/AE.pth",finetunning=False):
    epochs = 40
    dataloader_train_label = get_dataLoader_train(batch_size=32, shuffle=True)
    ...
    evaluate_encoder(model=model,dataloader_train_label=dataloader_train_label,dataloader_test=dataloader_test,device=device,epochs=epochs,patch_size=8,lr=1e-3,finetunning=finetunning)
...
def run_mae_load_eval_classification(path="outputs/model/MAE.pth"):
    ...
    model = MAE(img_size=96, patch_size=8, in_channels=3, embed_dim=768, decoder_embed_dim=576)

    print("chargement MAE")
    load_model(model, path, device)

    evaluate_encoder(model=model,dataloader_train_label=dataloader_train_label,dataloader_test=dataloader_test,device=device,epochs=epochs,patch_size=8,lr=1e-3,finetunning=False)
